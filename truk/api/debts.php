...
if ($method === 'GET') {
    if (isset($_GET['id'])) {
        ...
        if (isset($_GET['history'])) {
            $stmt = $pdo->prepare('SELECT * FROM debt_payments WHERE debt_id=? ORDER BY paid_at DESC, id DESC');
            $stmt->execute([$id]);
...
if ($method === 'POST') {
    ...
    $stmt = $pdo->prepare('INSERT INTO debts (driver, vehicle, type, amount, date, dueDate, status, paidAmount, lastPaidAt, paidOffAt, notes, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)');
    $stmt->execute([
        $d['driver'] ?? null,
        $d['vehicle'] ?? null,
        $d['type'] ?? 'hutang',
        ...
        $status,
        $paidAmount,
        okekirim_normalize_datetime($d['lastPaidAt'] ?? null),
        okekirim_normalize_datetime($d['paidOffAt'] ?? null),
        $d['notes'] ?? null,
        $createdAt
    ]);
    $id = $pdo->lastInsertId();
...
if (in_array($method, ['PUT', 'PATCH'])) {
    ...
    $stmt->execute([
        ...
        $status,
        $paidAmount,
        okekirim_normalize_datetime($lastPaidAt),
        okekirim_normalize_datetime($paidOffAt),
    ...
    if ($shouldLogPayment) {
        ...
        $paidAtSource = $paymentAmount !== null && $paymentAmount > 0
            ? ($d['lastPaidAt'] ?? $fallbackNow)
            : $fallbackNow;
        $paidAtValue = okekirim_normalize_datetime($paidAtSource) ?? $fallbackNow;

        $paymentCreatedAt = okekirim_normalize_datetime('now');
        $payStmt = $pdo->prepare('INSERT INTO debt_payments (debt_id, driver, amount, paid_at, notes, created_at) VALUES (?,?,?,?,?,?)');
        $payStmt->execute([
            $id,
