...
if ($m === 'POST') {
    ...
    $stmt = $pdo->prepare('INSERT INTO schedules (driver, vehicle, date, origin, destination, rit, orderType, fare, status, companyShare, paidCompanyAmount, notes, payment_notes, lastPaidAt, paidOffAt, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
    $stmt->execute([
        $d['driver'] ?? null,
        $d['vehicle'] ?? null,
        $d['date'] ?? null,
